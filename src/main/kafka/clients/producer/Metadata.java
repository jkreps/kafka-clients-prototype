...
public final class Metadata {
  ...
  /**
   * @param refreshBackoffMs The minimum amount of time that must expire between metadata refreshes to avoid busy polling
  ...
  /**
   * Fetch cluster metadata including partitions for the given topic. If there is no 
   * metadata for the given topic, block waiting for an update.
  ...
  /**
   * Does the current cluster info need to be updated?
   * An update is needed if it has been at least refreshBackoffMs since our last update 
   * and either
   * (1) an update has been requested or
   * (2) the current metadata has expired (more than metadataExpireMs has passed since the last refresh)
   */
