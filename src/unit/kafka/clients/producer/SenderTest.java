...
public class SenderTest {
  ...
  private Cluster cluster = new Cluster(asList(new Node(0, "localhost", 1969)), asList(new PartitionInfo("test", 0, 0, new int[0], new int[0])));
  private RecordAccumulator accumulator = new RecordAccumulator(batchSize, 1024*1024, 0L, time);
  ...
  public void testSimple() throws Exception {
    ...
    long offset = 42;
    selector.completeReceive(produceResponse(request.header().correlationId(), cluster.leaderFor(tp).id(), tp.topic(), tp.partition(), offset, Errors.NONE.code()));
    sender.run(time.milliseconds());
